  ...
  return menuItems.map((item) => (
    ...
    >
      ...
      <span className="relative flex items-center gap-1 sm:gap-2">
        {showIcons && <span className="text-xs sm:text-sm">{item.icon}</span>}
...
function ContactInfoBar({ theme, toggleTheme }) {
  ...
  return (
    <div className={`fixed top-0 w-full z-40 bg-[#3c6e71] text-white py-1 sm:py-2 text-xs sm:text-sm shadow-md transition-transform duration-300 ${
      isVisible ? "translate-y-0" : "translate-y-0"
    }`}>
      <div className="max-w-screen-xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 flex flex-col sm:flex-row justify-between items-center">
        <div className="flex items-center space-x-2 sm:space-x-4 mb-1 sm:mb-0">
          <a href="mailto:contact@example.com" className="flex items-center hover:text-[#3c6e71] transition-all duration-200 text-xs sm:text-sm group">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            <span className="hidden xs:inline">contact@example.com</span>
            <span className="xs:hidden">Email</span>
          </a>
          <div className="hidden sm:block text-white/60">|</div>
          <a href="tel:+1234567890" className="flex items-center hover:text-[#3c6e71] transition-all duration-200 text-xs sm:text-sm group">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          {[
            ...
            <a key={social.name} href="#" className="hover:text-[#3c6e71] transition-all duration-200 group relative">
              <svg 
                ...
                className="h-3 w-3 sm:h-4 sm:w-4 group-hover:scale-110 transition-all duration-200" 
                fill="currentColor" 
                viewBox="0 0 24 24"
              >
                <path d={social.path} />
              </svg>
              <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap">
                {social.name}
              </div>
            </a>
          ))}
          ...
          <div className="relative">
            <button
              onClick={toggleTheme}
              className="p-1 sm:p-2 rounded-full hover:bg-white/20 transition-all duration-300 group relative overflow-hidden"
              title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
            >
              <div className="relative z-10">
                {theme === "dark" ? (
                  <svg className="h-3 w-3 sm:h-4 sm:w-4 transform group-hover:rotate-12 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function MobileMenu({ isMenuOpen, setIsMenuOpen, activeLink, setActiveLink, theme, toggleTheme }) {
  return (
    <div className={`lg:hidden bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg border-t border-[#3c6e71]/50 dark:border-[#3c6e71]/50 shadow-xl transition-all duration-300 ease-in-out ${
      ...
      <div className="px-3 sm:px-4 py-4 sm:py-5 flex flex-col space-y-2 sm:space-y-3">
        ...
        <button
          ...
          className="mt-2 sm:mt-3 py-2 sm:py-3 px-4 sm:px-5 bg-[#3c6e71] text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:bg-[#2f5a5d] transition-all duration-300 text-sm sm:text-base group relative overflow-hidden"
        >
        ...
        <button
          onClick={toggleTheme}
          className="flex items-center justify-center gap-2 sm:gap-3 py-2 sm:py-3 px-3 sm:px-4 border-2 border-[#3c6e71]/50 dark:border-[#3c6e71]/50 rounded-xl text-[#000000] dark:text-gray-300 font-bold hover:bg-[#F5F5F5] dark:hover:bg-gray-800 hover:border-[#3c6e71] dark:hover:border-[#3c6e71] transition-all duration-300 text-sm sm:text-base mt-1 sm:mt-2 group"
        >
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <header
        className={`fixed top-8 sm:top-10 w-full z-30 transition-all duration-400 ${
          scrollActive
        ...
        <nav className="max-w-screen-xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 py-2 sm:py-3 flex items-center justify-between">
          <div className="flex flex-col items-start">
            ...
            >
              ...
              <span className="text-xl sm:text-2xl font-bold bg-[#3c6e71] bg-clip-text text-transparent group-hover:scale-105 transition-all duration-300">
                Portfolio
              </span>
              <div className="ml-1 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-[#3c6e71] rounded-full animate-pulse" />
            </LinkScroll>
          </div>

          <ul className="hidden lg:flex items-center space-x-0.5 sm:space-x-1 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl px-2 sm:px-4 py-1 sm:py-2 border border-[#3c6e71]/50 dark:border-[#3c6e71]/50">
            <CreateMenus
          ...
          <button
            className="lg:hidden flex flex-col justify-center items-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl text-[#000000] dark:text-gray-300 font-bold focus:outline-none shadow-md hover:shadow-lg transition-all duration-300 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-[#3c6e71]/50 dark:border-[#3c6e71]/50"
            onClick={() => setIsMenuOpen(!isMenuOpen)}
          >
            <span
              className={`block w-5 h-0.5 sm:w-6 sm:h-0.5 bg-current transform transition duration-300 ease-in-out ${
                isMenuOpen ? "rotate-45 translate-y-1.5" : ""
              }`}
            />
            <span
              className={`block w-5 h-0.5 sm:w-6 sm:h-0.5 bg-current transition-all duration-300 ease-in-out mt-1 sm:mt-1.5 ${
                isMenuOpen ? "opacity-0" : "opacity-100"
              }`}
            />
            <span
              className={`block w-5 h-0.5 sm:w-6 sm:h-0.5 bg-current transform transition duration-300 ease-in-out mt-1 sm:mt-1.5 ${
                isMenuOpen ? "-rotate-45 -translate-y-1.5" : ""
              }`}
            />
          </button>
